...
def get_f_std(y_std, gpr):
    """
    Computes the standard deviation of the function predictions.

    This function calculates the standard deviation of the Gaussian Process (GP)
    predictions after correcting for the noise in the training data. It uses
    the parameters from the GP model to determine the contribution of the noise
    and adjusts the predicted variance accordingly.

    Parameters:
    -----------
    y_std : numpy.ndarray
        The standard deviation of the predicted outputs from the GP model.
    
    gpr : GaussianProcessRegressor
        A trained Gaussian Process Regressor model from scikit-learn.

    Returns:
    --------
    f_std : numpy.ndarray
        The adjusted standard deviation of the function predictions, with noise
        contribution removed.

    Notes:
    ------
    This function assumes that the GP model uses a kernel with a noise component
    (e.g., RBF + WhiteKernel) and that the model has already been trained. The
    standard deviations are adjusted to account for the noise level in the training
    data, resulting in a more accurate representation of the uncertainty in the
